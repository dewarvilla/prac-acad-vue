...
// Endpoints
// -------------------------
// Si tu backend quedó en singular, cambia solo esta constante a '/auxilio'
...
async function getProducts(opts = {}) {
    ...
    try {
        const response = await api.get(API_AUX, {
...
const rules = {
    ...
    tasa_docente_cordoba_pernocta: [(v) => (v !== null && v !== '') || 'Requerido.', (v) => !Number.isNaN(Number(v)) || 'Debe ser numérico.', (v) => Number(v) >= 0 || 'Debe ser mayor o igual a 0.'],
    tasa_docente_fuera_pernocta: [(v) => (v !== null && v !== '') || 'Requerido.', (v) => !Number.isNaN(Number(v)) || 'Debe ser numérico.', (v) => Number(v) >= 0 || 'Debe ser mayor o igual a 0.'],
    factor_sin_pernocta: [(v) => (v !== null && v !== '') || 'Requerido.', (v) => !Number.isNaN(Number(v)) || 'Debe ser numérico.', (v) => Number(v) >= 0 || 'Debe ser mayor o igual a 0.', (v) => Number(v) <= 1 || 'Debe ser menor o igual a 1.'],
    umbral_km_fuera: [
        (v) => (v !== null && v !== '') || 'Requerido.',
        (v) => !Number.isNaN(Number(v)) || 'Debe ser numérico.',
        (v) => Number.isInteger(Number(v)) || 'Debe ser un entero.',
        (v) => Number(v) >= 0 || 'Debe ser mayor o igual a 0.',
        (v) => Number(v) <= 9999 || 'Debe ser menor o igual a 9999.'
    ],
    tasa_estudiante: [(v) => (v !== null && v !== '') || 'Requerido.', (v) => !Number.isNaN(Number(v)) || 'Debe ser numérico.', (v) => Number(v) >= 0 || 'Debe ser mayor o igual a 0.'],
    estado: []
};
...
<template>
    <div class="card">
        <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2"><h2 class="text-lg font-semibold m-0">Auxilios</h2></div>
        </div>
        ...
        <template v-else>
            <Toolbar class="mb-3">
                <template #start>
                    <div class="flex items-center gap-2 shrink-0">
                        <Button v-if="canCreateAuxilios" label="Crear" icon="pi pi-plus" @click="openNew" /> <Button v-if="canDeleteAuxilios" label="Borrar" icon="pi pi-trash" :disabled="!canBulkDelete" @click="confirmBulkDelete" />
            ...
            >
                <Column headerStyle="width:3rem">
                    ...
                    <template #checkbox="{ data, index }"> <Checkbox v-model="selected" :value="data" :inputId="`${tableUid}-row-${index + 1}`" name="row-select" :aria-label="`Seleccionar fila ${index + 1}`" /> </template>
                </Column>
                <Column field="acuerdo" header="Acuerdo" sortable style="min-width: 20rem; max-width: 30rem">
                    <template #body="{ data }">
                        <span class="block overflow-hidden text-ellipsis whitespace-nowrap" v-tooltip.top="data.acuerdo"> {{ data.acuerdo }} </span>
                    </template>
                </Column>
                <Column field="vigente_desde" header="Vigente desde" sortable style="min-width: 14rem; max-width: 16rem">
                    <template #body="{ data }"> {{ formatDate(data.vigente_desde) }} </template>
                </Column>
                <Column field="estado" header="Estado" sortable style="width: 12rem; max-width: 12rem">
                    <template #body="{ data }"> <Tag :value="estadoLabel(data.estado)" :severity="estadoSeverity(data.estado)" /> </template>
                </Column>
            ...
            <Dialog v-model:visible="productDialog" :header="product.id ? 'Editar auxilio' : 'Crear auxilio'" :style="{ width: '42rem' }" :modal="true">
                <div class="flex flex-col gap-4" @keydown.capture="onFormKeyCapture">
                    <div class="flex flex-col gap-2">
                        <label for="acuerdo">Acuerdo</label> <InputText id="acuerdo" v-model.trim="product.acuerdo" :invalid="showError('acuerdo')" @blur="onBlur('acuerdo')" fluid />
                        <small v-if="showError('acuerdo')" class="text-red-500"> {{ errors.acuerdo }} </small>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex flex-col gap-2">
                            <label for="vigente_desde">Vigente desde</label> <InputText id="vigente_desde" v-model.trim="product.vigente_desde" placeholder="YYYY-MM-DD" :invalid="showError('vigente_desde')" @blur="onBlur('vigente_desde')" fluid />
                            <small v-if="showError('vigente_desde')" class="text-red-500"> {{ errors.vigente_desde }} </small>
                        </div>
                        <div class="flex flex-col gap-2">
                            <label for="vigente_hasta">Vigente hasta</label> <InputText id="vigente_hasta" v-model.trim="product.vigente_hasta" placeholder="YYYY-MM-DD" :invalid="showError('vigente_hasta')" @blur="onBlur('vigente_hasta')" fluid />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <InputNumber id="tasa_estudiante" v-model="product.tasa_estudiante" :min="0" :maxFractionDigits="6" :useGrouping="false" :invalid="showError('tasa_estudiante')" @blur="onBlur('tasa_estudiante')" fluid />
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <InputNumber id="umbral_km_fuera" v-model="product.umbral_km_fuera" :min="0" :max="9999" :useGrouping="false" :invalid="showError('umbral_km_fuera')" @blur="onBlur('umbral_km_fuera')" fluid />
                            <small v-if="showError('umbral_km_fuera')" class="text-red-500"> {{ errors.umbral_km_fuera }} </small>
                        </div>
                    </div>
                    <div v-if="product.id" class="flex items-center gap-3 pt-2"><Checkbox id="estado" v-model="product.estado" :binary="true" /> <label for="estado">Activo</label></div>
                </div>
                <template #footer>
            ...
            <Dialog v-model:visible="deleteProductDialog" header="Confirmar" :style="{ width: '28rem' }" :modal="true">
                ...
                <template #footer> <Button label="No" icon="pi pi-times" text @click="deleteProductDialog = false" /> <Button label="Sí" icon="pi pi-check" severity="danger" @click="deleteProduct" /> </template>
            </Dialog>
            <Dialog v-model:visible="bulkDeleteDialog" header="Confirmar eliminado" :style="{ width: '28rem' }" :modal="true">
                ...
                <template #footer> <Button label="No" icon="pi pi-times" text @click="bulkDeleteDialog = false" /> <Button label="Sí" icon="pi pi-check" severity="danger" @click="bulkDelete" /> </template>
            </Dialog>
                    ...
                    <div v-for="d in details" :key="d.id" class="mb-4 border rounded-lg p-3 sm:p-4">
                        <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <div class="text-base font-semibold break-words leading-snug">{{ d.acuerdo }}</div>
                                <div class="text-sm text-surface-600 break-words">Configuración de auxilio vigente</div>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4 text-sm">
                            <div><b>Vigente desde:</b> {{ formatDate(d.vigente_desde) }}</div>
                            <div><b>Vigente hasta:</b> {{ formatDate(d.vigente_hasta) }}</div>
                            <div><b>Tasa docente Córdoba pernocta:</b> {{ formatFixed(d.tasa_docente_cordoba_pernocta) }}</div>
                            <div><b>Tasa docente fuera pernocta:</b> {{ formatFixed(d.tasa_docente_fuera_pernocta) }}</div>
                            <div><b>Tasa estudiante:</b> {{ formatFixed(d.tasa_estudiante) }}</div>
                            <div><b>Factor sin pernocta:</b> {{ formatFixed(d.factor_sin_pernocta) }}</div>
                            <div><b>Umbral km fuera:</b> {{ d.umbral_km_fuera ?? '—' }}</div>
                        </div>
